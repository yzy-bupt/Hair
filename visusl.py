...
parser.add_argument(
    ...
    type=str,
    help="evaluate at this precision",
    default="../dataset/CelebAMask-HQ/CelebA-HQ-img/3.jpg"
)
parser.add_argument(
    "--ref_mask_path",
    type=str,
    help="evaluate at this precision",
    default="../dataset/CelebAMask-HQ/CelebAMask-HQ-mask-anno/0/00003_hair.png"
)
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                ref_mask = np.array(ref_mask)[:,:,0][None,None]
                ref_mask = ref_mask.astype(np.float32)/255.0
                ref_mask[ref_mask < 0.5] = 0
                ...
                ref_mask_tensor = torch.from_numpy(ref_mask)
                ref_tensor = Resize([mask_tensor.shape[-2],mask_tensor.shape[-1]])(ref_tensor)
                ref_tensor = ref_tensor*ref_mask_tensor
                ref_tensor = Resize([224, 224])(ref_tensor)
                ...
                if not opt.skip_save:
                    for i,x_sample in enumerate(x_checked_image_torch):
                        ...
                        img.save(os.path.join(grid_path, 'grid-'+filename[:-4]+'_'+str(seed)+'_'+opt.ckpt[-9:-5]+'.png'))
...
with block:
    with gr.Row():
        gr.Markdown("## Control Stable Diffusion with Hair Style")
    with gr.Row():
        with gr.Column():
